(function() {
    ...
    var af = V.match(/Chrome/i) != null && V.match(/Version\/\d+\.\d+(\.\d+)?\sChrome\//i) == null ? true : false;
    ...
    var c = false;
    var v = false;
    var O = {closeUaAjax: false, keplerAjax: false, configCenterAjax: false};
    var g = {AROUSAL_APP: true, DOWNLOAD_LAYER: true, GENERAL_HEAD: true};
    var d = {keplerID: null, keplerFrom: 1, keplerParamJson: null};
    var u = {
        url: "mapi.php?format_=json&domain=" + encodeURI(document.referrer), method: "POST", async: false, timeout: 1000, withCredentials: true, hasAjax: "configCenterAjax", hasAjaxSend: false, error: function() {
            ...
            if (e && e.data) {
                for (var ak = 0; ak < e.data.length; ak++) {
                    var aj = e.data[ak];
                    if (aj.compent && !aj.display && g.hasOwnProperty(aj.compent)) {
    ...
    var N = {url: "https://so.m.jd.com/downLoad/closeUa.action?_format_=json", method: "POST", async: false, timeout: 1000, hasAjax: "closeUaAjax", hasAjaxSend: false, error: function() {
            O.closeUaAjax = true
        }, success: function(e) {
            O.closeUaAjax = true;
            e = JSON.parse(e);
    ...
    var ai = {url: "mapi.php?_format_=json", method: "POST", async: true, timeout: 1000, withCredentials: true, hasAjax: "keplerAjax", hasAjaxSend: false, error: function() {
    ...
    function I(e) {
        ...
        if ((i && ad >= 9) || Y || aa) {
            setTimeout(function() {
                ...
                an.setAttribute("href", am), an.style.display = "none", document.body.appendChild(an);
    ...
    function n(al) {
        ...
        var at = {category: "jump", des: "HomePage"};
        ...
        try {
            ...
            var ar = {mt_subsite: encodeURIComponent(k("mt_subsite")), __jdv: encodeURIComponent(k("__jdv")), unpl: encodeURIComponent(k("unpl")), __jda: encodeURIComponent(k("__jda"))};
        ...
        var am = {url: "//ccc-x.jd.com/dsp/op?openapp_url=" + encodeURI(ak) + "&openapp_source_type=100", method: "POST", async: true, timeout: 1000, error: function() {
            }, success: function(e) {
            }};
        K(am);
        return aj
    ...
    function w(e) {
        if (e.openAppBtnId && document.querySelector("#" + e.openAppBtnId)) {
            ...
            j(e.openAppBtnId, "click", function() {
                ...
                if (aj.openAppCallback) {
                    var ak = aj.openAppCallbackSource ? aj.openAppCallbackSource : null;
                    aj.openAppCallback.call(ak)
                }
                var am = {ajaxPrame: ai, funcList: function(ao) {
                        if (!ao.isnotWriteOpenAppCookie) {
                            var ap = ao.cookieFlag ? "downloadAppPlugIn_downCloseDate_" + ao.cookieFlag : "downloadAppPlugIn_downCloseDate";
                            s(ap, Date.now() + "_" + ao.appDownOpenIntervalTime, 60, "/", ".m.jd.com");
                            s(ap, Date.now() + "_" + ao.appDownOpenIntervalTime, 60, "/", "m.jd.hk")
                        }
                        X(ao);
                        I(ao)
                    }, funcPrame: aj};
        ...
        if (e.closePanelBtnId && e.closePanelId && document.querySelector("#" + e.closePanelId) && document.querySelector("#" + e.closePanelBtnId)) {
            ...
            if (Q(e)) {
                ...
                if (e.closeCallblack) {
                    ...
                    var aj = e.closeCallblackSource ? e.closeCallblackSource : null;
            ...
            j(e.closePanelBtnId, "click", function() {
                var ap = this.getAttribute("id");
                var al = R[ap];
                var ao = al.cookieFlag ? "downloadAppPlugIn_downCloseDate_" + al.cookieFlag : "downloadAppPlugIn_downCloseDate";
                if (!al.noRecord) {
                    s(ao, Date.now() + "_" + al.appDownCloseIntervalTime, 60, "/", "m.jd.com");
                    s(ao, Date.now() + "_" + al.appDownCloseIntervalTime, 60, "/", "m.jd.hk")
                }
                document.querySelector("#" + al.closePanelId).style.display = "none";
                if (al.closeCallblack) {
                    var an = event || window.event;
                    var am = al.closeCallblackSource ? al.closeCallblackSource : null;
    ...
    function Q(ak, aq) {
        ...
        var ap = null;
        if (aq) {
            ap = ak.cookieFlag ? "autoOpenApp_downCloseDate_" + ak.cookieFlag : "autoOpenApp_downCloseDate";
            if (v || !g.AROUSAL_APP) {
                al = true
            }
        } else {
            ...
            if (V.indexOf("Html5Plus") >= 0 || (aj >= 0 && aj != am) || c || !g.DOWNLOAD_LAYER) {
                al = true
            }
            ap = ak.cookieFlag ? "downloadAppPlugIn_downCloseDate_" + ak.cookieFlag : "downloadAppPlugIn_downCloseDate"
        }
        var an = k(ap);
        var ao = null;
        if (an) {
    ...
    function y(aj, ap, aw) {
        ...
        if (aq) {
            try {
                ...
                an.send(am);
                var al = {url: "//so.m.jd.com/downLoad/checkShield.action?_format_=json", method: "POST", async: true, timeout: 1000, error: function() {
                    }, success: function(e) {
                    }};
                K(al)
            } catch (ar) {
    ...
    function E(am, ak) {
        ...
        var e = true;
        if (!al && am && Object.prototype.toString.apply(am.ajaxPrame) === "[object Array]" && am.ajaxPrame.length > 0) {
        ...
        if (al && al.ua) {
            ...
            if (!v && al && al.autoCloseOsAndBrowser) {
                for (var ak = 0; ak < al.autoCloseOsAndBrowser.length; ak++) {
                    ...
                    if (e.os && e.browser && e.abtest && e.os == "IOS" && (a || B) && V.indexOf(e.browser) >= 0) {
                        ab(e.abtest);
                        break
                    }
                    if (e.os && e.browser && e.abtest && e.os == "Android" && ag && V.indexOf(e.browser) >= 0) {
                        ab(e.abtest);
                        break
                    }
                }
    ...
    function S(e) {
        var aj = W(e);
        w(aj);
        var ak = {ajaxPrame: [N, u], funcList: [U, y], funcPrame: aj};
        E(ak)
    }
    ...
    ac.downloadAppPlugInOpenApp = function(e) {
        ...
        var al = {ajaxPrame: [ai, N, u], funcList: function(am) {
                if (!Q(am, true)) {
                    cookieName = aj.cookieFlag ? "autoOpenApp_downCloseDate_" + aj.cookieFlag : "autoOpenApp_downCloseDate";
                    if (am.autoOpenIntervalTime) {
                        s(cookieName, Date.now() + "_" + am.autoOpenIntervalTime, 60, "/", ".m.jd.com");
                        s(cookieName, Date.now() + "_" + am.autoOpenIntervalTime, 60, "/", "m.jd.hk")
                    }
                    X(am);
